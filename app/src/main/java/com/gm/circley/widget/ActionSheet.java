...
public class ActionSheet extends Fragment implements OnClickListener {
	...
	private View createView() {
		...
		mBg.setId(R.id.bg_view);
		mBg.setOnClickListener(this);

		mPanel = new LinearLayout(getActivity());
		int navigationBarHeight = DisplayUtil.getNavigationBarHeight(getActivity());
	...
	private void createItems() {
		...
		if (titles != null) {
			for (int i = 0; i < titles.length; i++) {
				...
				bt.setId(R.id.cancel_button + i + 1);
	...
	public void onClick(View v) {
		if (v.getId() == R.id.bg_view && !getCancelableOnTouchOutside()) {
			return;
		}
		dismiss();
		if (v.getId() != R.id.cancel_button && v.getId() != R.id.bg_view) {
			if (mListener != null) {
				mListener.onOtherButtonClick(this, v.getId() - R.id.cancel_button - 1);
			}
			isCancel = false;
		}
	}
